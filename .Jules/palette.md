...
## 2026-06-10 - Progress Bar ARIA
**Learning:** Progress bars implemented as purely visual div elements with width percentages are invisible to screen readers, leaving users unaware of completion status.
**Action:** Use `role="progressbar"` with `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` on the container. Add an `aria-label` describing what is being measured.
