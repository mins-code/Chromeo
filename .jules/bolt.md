...
## 2024-06-06 - Caching Intl.DateTimeFormat
**Learning:** `Date.prototype.toLocaleDateString()` and `toLocaleTimeString()` are extremely slow when called repeatedly in tight loops due to internal `Intl` object instantiation overhead.
**Action:** Cache `Intl.DateTimeFormat` instances outside of formatting functions and use `.format(date)` instead. Remember to explicitly handle invalid dates with `Number.isNaN(d.valueOf())` as `.format()` will throw a `RangeError` otherwise.
