...
## 2024-06-14 - Optimizing inline array reduction chains in React JSX
**Learning:** Combining multiple `.filter().reduce()` chains over the same array into a single `.reduce()` pass, and extracting inline JSX reductions into a single memoized `useMemo` object, significantly reduces CPU overhead from O(K*N) to O(N) during frequent render cycles.
**Action:** Always scan for redundant array iterations (especially in UI display calculations) and consolidate them into a single-pass accumulator block outside the return statement, ideally cached with `useMemo`.
