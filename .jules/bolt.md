...
## 2024-04-16 - Cached Intl.DateTimeFormat and optimized date formatting
**Learning:** Using `Intl.DateTimeFormat` dynamically within loops and `toLocaleDateString`/`toLocaleTimeString` without caching introduces massive overhead (O(N) object instantiation). Furthermore, `.toISOString().split('T')[0]` is much slower than using native UTC methods (`getUTCFullYear`, `getUTCMonth`, `getUTCDate`) for generating string-based keys.
**Action:** Always pre-instantiate and cache `Intl.DateTimeFormat` for UI-facing date formatting, and prefer native UTC methods combined with simple string concatenation (with inline padding like `month < 10 ? '0' : ''`) over intermediate array/string allocations (e.g. `split()`) for performance-sensitive date keys.
