...
## 2024-06-02 - Combine Multiple Array Reductions
**Learning:** Components computing multiple aggregations over the same array (e.g., both total income and total expenses) inline or in separate `useMemo` blocks trigger redundant O(N) operations. Using chained `.filter().reduce()` makes this even worse.
**Action:** Combine them into a single `.reduce()` pass that returns an object with multiple accumulators, reducing iteration overhead from O(K*N) to O(N).
