...
## 2024-06-26 - Optimized multiple array reductions in JSX
**Learning:** Inline array `.filter().reduce()` chains inside React JSX render functions cause redundant O(K*N) calculations on every single render, severely impacting performance for data-heavy components.
**Action:** Extract these calculations into a single O(N) pass `.reduce()` inside a `useMemo` block to compute multiple aggregations simultaneously and avoid redundant calculations on re-renders.
