...
**Learning:** Calling `Intl.DateTimeFormat().format()` or `.toLocaleDateString()` inside tight React loops (like calendar rendering) generates immense object instantiation overhead and can be >10x slower than cached instances. Similarly, generating string keys (e.g. `YYYY-MM-DD`) using `.toISOString().split('T')[0]` allocates unneeded strings and arrays, whereas native `.getUTCFullYear()` and manual string concatenation is vastly faster (~14x).
**Action:** Always pre-allocate and reuse `Intl.DateTimeFormat` objects for displaying dates in UI loops, and utilize native Date getter methods and template literals for generating keys in performance-critical paths.
