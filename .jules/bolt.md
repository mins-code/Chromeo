...
## 2024-05-24 - Optimize Date formatting and Array reductions
**Learning:** Instantiating `Intl.DateTimeFormat` dynamically inside formatting functions or chaining `Array.filter().reduce()` inside React components creates substantial performance overhead in list-heavy rendering, as previously discovered and re-confirmed. `String.prototype.padStart` vs ternary operator for zero-padding was benchmarked and ternary was shown to be faster. Memory rule applies: Use cached `Intl.DateTimeFormat` and native getter + string concat for dates. Use a single `.reduce()` for conditional summation.
**Action:** Replaced usage of `.filter().reduce()` in `web/components/BudgetPlanner.tsx` to a single-pass `.reduce()`, effectively cutting down iteration complexity in tight loops. Replaced `toLocaleString`, `toLocaleDateString`, `toLocaleTimeString` calls in `web/utils/date.ts` with cached `Intl.DateTimeFormat` instances. Also replaced `toISOString().split('T')[0]` with native getUTC* and ternary zero-padding.
