...
## 2025-03-09 - [Optimizing O(Days * N) Nested Loops]
**Learning:** When grouping large datasets by date (e.g., in CustomIntervalView), O(Days * N) nested loops using Array.filter inside an outer loop over days cause redundant iterations.
**Action:** Refactor into an O(N) single-pass operation using a hash map to drastically reduce redundant iterations and improve React rendering performance.
