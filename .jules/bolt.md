...
## 2026-06-20 - Optimize repeated array aggregations in React JSX

**Learning:** The application was performing expensive array aggregations (e.g., chained `.filter().reduce()`) directly inside JSX render blocks, often repeating the exact same calculation multiple times for display values, conditional classes, and progress bars. This caused O(K*N) operations on every single re-render.
**Action:** When a component computes multiple aggregations over the same array or repeats an identical calculation in JSX, combine them into a single `.reduce()` pass outside the render cycle and memoize the result using `useMemo` to reduce iteration overhead to O(N).
