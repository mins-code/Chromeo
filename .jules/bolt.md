...
## 2026-03-05 - Fast Date Formatter Caching
**Learning:** Instantiating `Intl.DateTimeFormat` or using `toLocaleDateString()` and `toISOString().split('T')[0]` inside tight React render loops adds significant overhead and slows down list rendering.
**Action:** Extract standard `Intl.DateTimeFormat` configurations to module-scope cached instances. Replace `.toISOString().split('T')[0]` with native JavaScript string concatenation (`getUTCFullYear()`, `getUTCMonth()`, `getUTCDate()`) for `YYYY-MM-DD` generation, avoiding object creation and string allocations.
