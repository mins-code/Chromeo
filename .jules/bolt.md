...
## 2026-06-03 - Optimize chained filter and reduce operations
**Learning:** When a component computes multiple aggregations over the same array (e.g., calculating both total income and total expenses from a transactions list), using chained `.filter().reduce()` results in multiple iterations (O(K*N)).
**Action:** Combine them into a single `.reduce()` pass that returns an object with multiple accumulators. This reduces iteration overhead from O(K*N) to O(N). Also, extract inline array reductions into a single memoized variable (`useMemo`) to avoid redundant O(N) operations per render.
