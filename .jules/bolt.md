...
## 2026-06-08 - Single-Pass Aggregation Over Multiple Passes
**Learning:** When a component computes multiple aggregations over the same array (e.g., calculating both total income and total expenses from a transactions list, or performing multiple inline `.filter().reduce()` chains in JSX), doing them separately executes O(K*N) operations. This creates unnecessary CPU overhead on every re-render.
**Action:** Combine multiple aggregations into a single `.reduce()` pass that returns an object with multiple accumulators, reducing iteration overhead to O(N).
