...
## 2026-04-24 - Extract JSX Array Reductions
**Learning:** Inline array reductions (like chained `.filter().reduce()`) inside React JSX render functions execute O(K*N) operations on every re-render, blocking the main thread during component updates.
**Action:** Extract these into a single O(N) pass loop within a `useMemo` block to minimize CPU overhead.
