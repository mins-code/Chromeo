...
## 2026-03-05 - Combined Array Aggregations
**Learning:** When a component computes multiple aggregations over the same array (like calculating total income and expenses), using separate `.filter().reduce()` chains results in O(K*N) iterations and multiple intermediate array allocations. In large datasets, this redundancy compounds inside React renders.
**Action:** Combine them into a single `.reduce()` pass returning an object with multiple accumulators, reducing iteration overhead to O(N).
