...
## 2025-03-23 - Extract repeated array calculations into useMemo
**Learning:** Extracting repeated inline array calculations (like multiple `.filter().reduce()` chains over the same large list) within React render loops into a single `useMemo` hook significantly reduces redundant O(N) work and improves component rendering performance.
**Action:** When working on complex UI components (like dashboards or complex lists), always identify redundant array iterations in JSX render output and lift them into memoized values.
