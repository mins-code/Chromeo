...
## 2026-05-23 - O(N) Array Iteration Reduction in JSX
**Learning:** React component JSX sometimes duplicates expensive inline calculations for different parts of the same UI (like using the same `reduce` result for an amount, a remaining balance, and a progress bar width). This executes the exact same O(N) iteration multiple times on every single re-render.
**Action:** Always extract repeated O(N) operations out of JSX, calculate them once with single-pass `.reduce()` or `useMemo`, and reference the primitive variable.
