...
## $(date +%Y-%m-%d) - Inline array reductions to prevent intermediate array allocations
**Learning:** Chained `.filter(...).reduce(...)` operations inside React render cycles cause O(2N) iteration and intermediate array allocation overhead.
**Action:** Replace `.filter(condition).reduce(sum)` with a single-pass inline `.reduce((acc, curr) => condition ? acc + curr : acc)` to minimize CPU and memory overhead, especially for larger arrays. If calculated multiple times, extract the calculation to a `useMemo` block.
