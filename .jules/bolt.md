...
## 2024-04-02 - Date string generation optimization
**Learning:** Using `toISOString().split('T')[0]` is unnecessarily slow (~1.3s for 1M loops) for generating YYYY-MM-DD date strings in tight loops due to object allocations and array operations.
**Action:** Use native JavaScript Date UTC methods (`getUTCFullYear()`, `getUTCMonth()`, `getUTCDate()`) with inline ternary zero-padding string concatenation instead, which is ~7x faster (~165ms for 1M loops) and preserves UTC behavior without the overhead.
