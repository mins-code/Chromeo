...
## 2025-03-08 - Intl.DateTimeFormat caching & Native Date Concatenation
**Learning:** Re-instantiating `Intl.DateTimeFormat` via `.toLocaleDateString()` or `.toLocaleTimeString()` is a severe O(N) bottleneck during large list rendering (~26x slower than caching). Additionally, creating YYYY-MM-DD keys using `.toISOString().split('T')[0]` evaluates to UTC (risking timezone offset bugs) and is significantly slower (~7x slower) than native direct date part concatenation (`${year}-${month}-${day}`).
**Action:** Always cache `Intl.DateTimeFormat` instances outside of render/formatting loops. For generating YYYY-MM-DD local date keys, use native JavaScript Date methods (`getFullYear()`, `getMonth()`, `getDate()`) with string concatenation and `padStart(2, '0')` rather than generic string manipulation on ISO representations.
