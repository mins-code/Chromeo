...
## 2024-05-24 - CustomIntervalView Rendering Optimizations
**Learning:** In timeline-based UI views (like CustomIntervalView) that update every minute due to a clock tick, the entire grid and all task cells will indiscriminately re-render unless explicitly memoized. Additionally, using `date-fns` `format()` repeatedly inside a tight loop across tasks and days creates noticeable garbage collection overhead compared to using native string building with `padStart()`.
**Action:** Use `React.memo` on stateless/stable child components (like hour cells or task blocks) and use hash maps combined with native string date-key construction (e.g. `YYYY-MM-DD`) to shift grouping operations from O(Days * Tasks) down to O(Tasks).
