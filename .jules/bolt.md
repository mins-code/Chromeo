...
## 2026-03-05 - Avoid O(K*N) Inline Array Aggregations
**Learning:** When a component computes multiple aggregations over the same array (e.g., total income and total expenses) inline in the JSX or in separate `.filter().reduce()` chains, it executes O(K*N) operations on every re-render.
**Action:** Extract and combine these into a single `.reduce()` pass returning an object with multiple accumulators within a `useMemo` block. This reduces iteration overhead to O(N) and minimizes CPU overhead during rendering.
