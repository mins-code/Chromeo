...
## 2025-01-08 - Hoist Date instantiations out of nested loops
**Learning:** Parsing `Date` objects inside nested O(N*M) loops (like checking recurring dates against a daily forecast) is extremely slow and allocates excessive memory.
**Action:** Pre-parse the dates into primitive components (day, month, dayOfWeek) before the loop to achieve a ~3x speedup. Also, replace `toISOString().split('T')[0]` with native UTC getters + ternary padding.
