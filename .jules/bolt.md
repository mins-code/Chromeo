...
## 2025-01-20 - Caching Intl.DateTimeFormat and avoiding toISOString for performance
**Learning:** Caching `Intl.DateTimeFormat` instances for date and time formatting (e.g., in `web/utils/date.ts`) is significantly faster than using `date-fns` `format()` or un-cached `toLocaleDateString()`/`toLocaleTimeString()` inside tight React render loops, avoiding severe O(N) object instantiation overhead (yielding ~50-100x speedups) during list rendering. Explicitly handle invalid dates by returning 'Invalid Date' since `Intl.DateTimeFormat.format()` throws a `RangeError`. Also, replacing `toISOString().split('T')[0]` with native JavaScript Date UTC methods (`getUTCFullYear()`, `getUTCMonth()`, `getUTCDate()`) and inline ternary padding (`month < 10 ? '0' : ''`) is significantly faster (~7x) for generating YYYY-MM-DD date keys, avoiding string and array allocation overhead. Ensure invalid dates throw a `RangeError` to match native `toISOString()` behavior.
**Action:** Always cache `Intl.DateTimeFormat` objects outside functions to avoid repeated object instantiation in hot loops. Always prefer native Date methods with string concatenation for simple fixed date formats.
