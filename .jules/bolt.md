...
## 2026-03-05 - Combined Accumulator for React JSX Loop Optimizations
**Learning:** A component frequently repeating multiple aggregations over an identical list inside JSX elements (e.g. `list.filter().reduce()` repeated multiple times for different properties) performs redundant O(K*N) passes on every render.
**Action:** Extract these into a single O(N) pass loop using `.reduce()` inside a `useMemo` that returns an object with multiple accumulators, drastically reducing the component's render overhead time.
