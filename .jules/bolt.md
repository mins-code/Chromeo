...
## 2026-06-24 - Single Pass Reduce for Multiple Aggregations
**Learning:** When a component computes multiple aggregations over the same array (e.g., calculating both total income and total expenses from a transactions list), using multiple chained `.filter().reduce()` calls iterates over the array multiple times. Furthermore, inline array reductions inside React JSX render functions execute on every re-render, creating O(K*N) operations.
**Action:** Combine multiple aggregations into a single `.reduce()` pass that returns an object with multiple accumulators, and extract inline reductions into a `useMemo` block outside the render return statement to minimize CPU overhead from O(K*N) to O(N).
