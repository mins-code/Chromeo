...
## 2026-03-05 - Combined Array Reductions
**Learning:** When a component computes multiple aggregations over the same array (e.g., calculating both total income and total expenses from a transactions list), using multiple chained `.filter().reduce()` calls results in O(K*N) iteration overhead. Additionally, performing this inline inside JSX causes it to run on every re-render.
**Action:** Combine them into a single `.reduce()` pass that returns an object with multiple accumulators, reducing iteration overhead to O(N). Also, wrap this inside `useMemo` to prevent re-calculations during renders.
