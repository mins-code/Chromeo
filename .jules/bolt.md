...
## 2024-05-16 - Fast YYYY-MM-DD Date Key Generation
**Learning:** Using `.toISOString().split('T')[0]` to generate date keys creates unnecessary string arrays and allocations, adding significant performance overhead in tight loops.
**Action:** Replace it with native Date UTC methods (`getUTCFullYear`, `getUTCMonth`, `getUTCDate`) and inline ternary padding to generate date keys efficiently while preserving UTC behavior.
