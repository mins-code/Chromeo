...
## 2026-03-01 - Orphaned PII via Hardcoded Deletion Array
**Vulnerability:** The account deletion edge function looped over a hardcoded array of tables, blindly deleting records where `user_id` matched the departing user. However, many relationship tables (`partnerships`, `budget_shares`, `note_shares`, `teams`) used different foreign key names (`owner_id`, `partner_id`, `user_id_1`, etc.). Because these generic `.eq("user_id", userId)` loops were wrapped in a `try/catch`, they silently failed on these tables, leaving orphaned, personally identifiable information (PII) in the database and violating GDPR data erasure requirements.
**Learning:** Hardcoded, monolithic arrays for cascaded deletions are brittle and prone to schema-drift vulnerabilities. Additionally, swallowing errors on deletion queries masks critical data retention bugs.
**Prevention:** Explicitly handle complex relationship cleanups outside of generic loops using `.or()` filters to catch all foreign key associations. Ideally, utilize database-level `ON DELETE CASCADE` constraints directly tied to the core `profiles` or `auth.users` tables to guarantee atomic data removal without relying on manual application layer loops.
