...
## 2025-02-23 - GDPR Account Deletion Foreign Key Gap
**Vulnerability:** The account deletion edge function failed to delete user data in tables that lacked a direct `user_id` column (e.g., `note_shares`, `day_plans`, `notes`, `partnerships`, `budget_shares`), leading to orphaned user data and GDPR non-compliance.
**Learning:** Automated deletion routines that iterate over a hardcoded array and filter solely by `user_id` are insufficient. Schema additions of tables with alternative foreign keys (e.g., `owner_id`, `partner_id`, `user_id_1`) silently bypass the deletion loop if not manually added to specialized cleanup blocks.
**Prevention:** Whenever a new table containing user data is added to the database schema, explicitly verify whether its foreign keys strictly match the columns targeted in the account deletion function. If not, the function's deletion array or custom cleanup logic must be manually updated.
