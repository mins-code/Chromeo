[Output truncated for brevity]
 ...
 itself is allowed. The escape character can be used to neutralize the sanitizer's work.
...
## 2026-03-24 - GDPR Incomplete Account Deletion
**Vulnerability:** The `account-deletion` Edge Function used a hardcoded array of `tablesToDelete` to remove user data. Newly added tables (`notes`, `day_plans`) were missing from this list, causing their data to persist after the user deleted their account.
**Learning:** Hardcoded arrays defining "all user data tables" are fragile and easily become outdated as the database schema evolves, leading to incomplete data deletion (a severe GDPR violation).
**Prevention:**
1. Establish a process or automated test to ensure any new table containing user data is appended to the deletion list.
2. For shared resources (like `note_shares`), explicitly handle deletion using all relevant foreign keys (e.g., `owner_id` and `shared_with_id`) during the cleanup phase.