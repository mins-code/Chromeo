...
## 2024-05-24 - Fix Incomplete Account Deletion
**Vulnerability:** User data in tables without a direct `user_id` column (like `note_shares`, `partnerships`, `budget_shares`, and `teams`) was not being fully deleted when an account was deleted, violating data privacy regulations and leading to orphaned data.
**Learning:** Generic deletion loops iterating over a hardcoded array assuming a uniform `user_id` column are fragile and lead to incomplete deletions for complex schemas with multiple foreign keys or different naming conventions. The `note_shares` and `notes` tables were also completely missing from the deletion logic.
**Prevention:** Always maintain a comprehensive and explicitly handled data deletion process. Map all data relations and explicitly use the `.or()` filter syntax when a user ID might match multiple different foreign key columns (e.g., `owner_id` or `shared_with_id`).
