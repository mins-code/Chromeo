...
## 2026-06-27 - Search Path Hijacking in SECURITY DEFINER Functions
**Vulnerability:** Multiple PostgreSQL functions (`handle_new_user`, `is_team_owner`, `is_team_member`, `is_team_admin`) were defined with `SECURITY DEFINER` but omitted an explicit `SET search_path` configuration. This allows attackers to create malicious objects in schemas that appear earlier in the search path (like `public` if not controlled), overriding standard functions or operators executing with elevated privileges.
**Learning:** `SECURITY DEFINER` functions run with the privileges of the creator, which bypasses RLS and grants high access. If the search path is not explicitly set, the function inherits the caller's search path, leading to potential privilege escalation.
**Prevention:** Always include `SET search_path = public` (or `''` to force explicit schema qualifications) when creating or replacing `SECURITY DEFINER` functions.
