...
## 2024-05-27 - [GDPR Account Deletion Completeness]
**Vulnerability:** The account deletion edge function missed user data in tables created by newer migrations (`notes`, `day_plans`) and failed to delete relationship rows lacking a direct `user_id` column (`note_shares`, `partnerships`, `budget_shares` where user is partner). This leaves orphaned PII and violates GDPR complete data removal requirements.
**Learning:** Hardcoded application-layer data cleanup arrays must be manually kept in sync with database schema migrations. Furthermore, tables with custom foreign keys (like `owner_id`, `partner_id`, `shared_with_id`, `user_id_1`) silently fail deletion when iterating with `.eq('user_id', userId)`.
**Prevention:** Always verify schema additions against data deletion logic. Favor database-level `ON DELETE CASCADE` linked to the core `auth.users` or `profiles` tables instead of maintaining manual arrays where possible.
