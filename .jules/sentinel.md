...
## 2026-02-21 - Incomplete GDPR Account Deletion
**Vulnerability:** The account deletion edge function hardcoded the tables to delete, but failed to include newly added tables (`notes`, `note_shares`, `day_plans`) and failed to delete records where the user was a foreign key other than `owner_id` or `user_id` (e.g. `partner_id` in `budget_shares`, `user_id_1`/`user_id_2` in `partnerships`, `shared_with_id` in `note_shares`). This results in orphaned personal data remaining in the database after an account is deleted.
**Learning:** Hardcoded deletion logic is prone to desync with schema updates. When relying on manual deletion instead of `ON DELETE CASCADE` for compliance, every new table or foreign key relationship must be explicitly mapped in the deletion function.
**Prevention:** Favor `ON DELETE CASCADE` on foreign keys referencing `auth.users` or `profiles`. If manual deletion is required, audit the schema periodically for all tables referencing user IDs and use `.or()` filters for tables with multiple user relationships.
