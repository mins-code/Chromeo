...
## 2024-07-04 - GDPR Data Retention Vulnerability in Account Deletion
**Vulnerability:** The account deletion logic failed to completely remove user data from newer tables (`notes`, `day_plans`, `note_shares`) and tables without a direct `user_id` column (`partnerships`, `budget_shares`, `teams`), due to outdated hardcoded table arrays and insufficient shared try/catch error handling.
**Learning:** Hardcoded arrays for schema cleanup are fragile and prone to missing newly added tables. Relying on a single shared try/catch for multiple deletion operations causes a single failure to silently skip subsequent deletions, violating GDPR data retention policies.
**Prevention:** Use explicitly separate `try...catch` blocks for each independent cleanup operation. Use the `.or()` filter syntax to ensure comprehensive deletion across all relevant foreign keys (e.g., `owner_id`, `partner_id`, `shared_with_id`).
