...
## 2024-05-24 - Fix Orphaned PII Data Leak on Account Deletion
**Vulnerability:** The account deletion edge function used a generic loop to delete records via `.eq("user_id", userId)`. Tables lacking a direct `user_id` column (like `budget_shares`, `partnerships`, `note_shares`, `teams`) silently threw errors in the try/catch block, leaving user data orphaned in the database after account deletion, violating data privacy regulations.
**Learning:** Hardcoded arrays for cascading deletions are brittle and prone to silent failures if schemas diverge from the assumed `user_id` naming convention. When using Supabase `try/catch` loops for cleanup, silent failures obscure data leaks.
**Prevention:** Explicitly handle complex relational tables using exact foreign keys (e.g., `owner_id`, `partner_id`, `shared_with_id`) with `.or()` queries, or ideally rely on database-level `ON DELETE CASCADE` referring to `auth.users(id)` where possible.
