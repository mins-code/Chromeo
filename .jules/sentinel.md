...
## 2024-05-23 - Incomplete Account Deletion for New Tables
**Vulnerability:** The account deletion edge function hardcoded the tables to delete but wasn't updated when new tables (`notes`, `note_shares`, `day_plans`) were added. This leads to incomplete data removal and GDPR violations.
**Learning:** The hardcoded deletion arrays create a high risk of data remaining on the server when new features are deployed if not strictly monitored.
**Prevention:** Maintain automated tests or checks to ensure all database tables are accounted for in the account deletion logic.
