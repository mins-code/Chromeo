...
## 2026-06-12 - Incomplete GDPR Data Deletion
**Vulnerability:** The account deletion edge function used a hardcoded array of tables to delete data, which became outdated as new tables (`day_plans`, `notes`) were added to the schema. Additionally, tables lacking a direct `user_id` column (`teams`, `partnerships`, `budget_shares`, `note_shares`) were failing to delete properly and silently failing because Supabase queries do not throw exceptions on errors by default.
**Learning:** Hardcoded schema lists in application code inevitably fall out of sync with the database, leading to GDPR violations via orphaned data. Silently failing database queries obscure critical security and compliance failures.
**Prevention:** Use `.throwOnError()` or explicitly check the `error` object on all Supabase client deletions. For GDPR compliance, favor database-level `ON DELETE CASCADE` constraints attached to the central `profiles` or `auth.users` table rather than manually maintaining deletion lists.
