...
## 2024-05-04 - Incomplete Data Deletion Vulnerability (GDPR Risk)
**Vulnerability:** The account deletion edge function hardcoded an array of tables (`tablesToDelete`) relying strictly on a `user_id` foreign key. Tables introduced later (`notes`, `day_plans`) or tables with non-standard foreign keys (`note_shares` using `owner_id`/`shared_with_id`, `partnerships` using `user_id_1`/`user_id_2`) were orphaned, resulting in incomplete account data deletion (a high-severity GDPR violation).
**Learning:** Whenever the database schema is updated to add tables associated with a user, automated deletion scripts must be manually updated to ensure complete data removal.
**Prevention:** Avoid hardcoding `tablesToDelete` or explicitly implement foreign key deletions (`.or('owner_id.eq.val,shared_with_id.eq.val')`) for complex relationships when adding new tables to ensure comprehensive GDPR compliance.
