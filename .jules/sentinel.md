...
## 2026-03-05 - Incomplete Account Deletion (GDPR Violation)
**Vulnerability:** The account deletion function looped over an array of tables using `.eq('user_id', userId)`, silently failing on tables that lacked a `user_id` column (like `teams`, `partnerships`, `budget_shares`, and `note_shares`), leaving orphaned PII.
**Learning:** Hardcoded deletion logic that assumes a uniform schema (`user_id` everywhere) fails when new tables use different foreign keys (`owner_id`, `partner_id`, etc). Catching these errors silently hides the data leak.
**Prevention:** Tables lacking a direct `user_id` column must be explicitly handled in separate deletion blocks using `.or()` to match all relevant foreign keys, ensuring comprehensive data removal.
