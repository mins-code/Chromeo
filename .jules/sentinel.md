...
## 2026-03-05 - Incomplete GDPR Data Deletion (Orphaned Data)
**Vulnerability:** The account deletion edge function used a generic loop over a hardcoded array of tables to delete records matching `user_id`. It failed to properly delete records in tables lacking a direct `user_id` column (like `note_shares`, `partnerships`, `budget_shares`, and `teams`), leaving orphaned user data in the database and violating data deletion regulations.
**Learning:** Hardcoded deletion arrays are brittle and prone to omitting newer tables or tables with non-standard foreign keys (`owner_id`, `shared_with_id`). Furthermore, deleting parent tables before their children (like `team_members` before `teams`) can cause foreign key constraint failures.
**Prevention:** Tables lacking a direct `user_id` column must be explicitly handled in separate deletion blocks using their relevant foreign keys (e.g., using `.or('owner_id.eq.val,shared_with_id.eq.val')`). These explicit blocks must execute *after* the generic `tablesToDelete` loop to ensure dependent child tables are deleted first. Ideally, use `ON DELETE CASCADE` on database foreign keys referencing `auth.users` to let the database engine handle cleanup automatically.
