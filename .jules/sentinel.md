...
## 2024-11-20 - Fix Overly Permissive CORS Configuration in Account Deletion Edge Function
**Vulnerability:** The `account-deletion` Edge Function hardcoded `Access-Control-Allow-Origin: "*"` in its CORS headers, which allowed any origin to make cross-origin requests to this sensitive endpoint. While CSRF is mitigated by the requirement of an `Authorization` header, wildcard CORS misconfiguration creates unnecessary attack surface and violates the principle of least privilege, especially for critical actions like account deletion.
**Learning:** Some Edge Functions were using a static, overly permissive CORS header (`*`), whereas others correctly implemented dynamic origin validation. The lack of standardized CORS enforcement across all sensitive endpoints exposed the deletion functionality to potential cross-origin attacks or unintended integrations.
**Prevention:** All Supabase Edge Functions must implement strict dynamic origin validation by checking the request's `Origin` against a whitelist of trusted domains (e.g., `APP_URL`, `localhost`). The server must respond with `Access-Control-Allow-Origin: <Validated_Origin>` and include `Vary: Origin` to prevent cache poisoning. Wildcard CORS (`*`) should never be used on endpoints performing authenticated or sensitive actions.
