...
## 2026-05-09 - Incomplete Account Deletion Data Cleanup
**Vulnerability:** The `account-deletion` edge function failed to delete user data in tables referencing the user ID with custom column names (e.g. `owner_id`, `partner_id`, `shared_with_id`, `user_id_1`, `user_id_2`). This left orphaned personal data, violating data retention and GDPR compliance rules.
**Learning:** Tables lacking a direct `user_id` column (e.g., `note_shares`, `partnerships`, `budget_shares`) must be explicitly handled across all their relevant foreign keys when a user deletes their account. Using the Supabase `.or()` filter ensures comprehensive coverage. Also, manually hardcoded list of tables (`tablesToDelete`) can drift out of sync when the schema evolves (like adding `notes` or `day_plans`).
**Prevention:** Whenever new tables with user data are introduced, the `tablesToDelete` list and associated manual cleanup blocks in deletion scripts must be explicitly updated. Validate all foreign key relations associated with the user.
