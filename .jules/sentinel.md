...
## 2026-07-08 - Search Path Hijacking in PostgreSQL
**Vulnerability:** PostgreSQL `SECURITY DEFINER` functions did not specify a `SET search_path = public` configuration.
**Learning:** Functions executed with elevated privileges (`SECURITY DEFINER`) can be vulnerable to search path hijacking if an attacker creates malicious objects (like operators or functions) in a schema that precedes the intended schema in the search path.
**Prevention:** Always append `SET search_path = public` (or an empty string `''` to force explicit schema qualification) when creating or altering `SECURITY DEFINER` functions.
