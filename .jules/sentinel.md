...
## 2026-01-24 - [CRITICAL] Fix orphaned user data after account deletion
**Vulnerability:** The account deletion Edge Function (`supabase/functions/account-deletion/index.ts`) lacked comprehensive deletion routines for the newly introduced `day_plans`, `notes`, and `note_shares` tables.
**Learning:** Hardcoded arrays defining tables to delete based on the `user_id` foreign key must be manually updated whenever new tables containing user data are added to the database schema. Additionally, tables using different foreign key structures (like `note_shares` which uses `owner_id` and `shared_with_id`) will fail silent foreign key cascades if not handled explicitly.
**Prevention:** In the future, explicitly document table deletion responsibilities alongside database migrations. Alternatively, investigate using Supabase database triggers to handle dependent cascading deletes across the entire database autonomously, removing the need to manage a hardcoded list of tables in Edge Functions.
