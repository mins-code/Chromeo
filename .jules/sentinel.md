...
## 2024-03-05 - Restrict CORS Configuration in Account Deletion Edge Function
**Vulnerability:** The `account-deletion` Edge Function had an overly permissive CORS configuration (`Access-Control-Allow-Origin: *`). While the endpoints require authentication, a wildcard origin poses a risk if browsers auto-attach credentials or if tokens are somehow available cross-domain. More importantly, returning a wildcard without `Vary: Origin` can lead to cache poisoning issues.
**Learning:** This is a common oversight where edge functions handling sensitive operations (like account deletion) are created using a boilerplate that doesn't restrict origins.
**Prevention:** Always validate the `Origin` header dynamically against a whitelist of trusted domains (e.g., `APP_URL`, `http://localhost:3000`) and use `Vary: Origin` in CORS headers to ensure browsers correctly cache responses per origin.
