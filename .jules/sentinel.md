...
## 2026-03-08 - Incomplete Data Deletion (GDPR)
**Vulnerability:** The account deletion function iterated through a hardcoded list of tables and attempted to delete records based exclusively on the `user_id` column. However, several tables (`teams`, `budget_shares`, `partnerships`, `note_shares`, `team_members`) utilized different foreign keys (e.g., `owner_id`, `partner_id`, `user_id_1`, `shared_with_id`) or lacked a `user_id` entirely. Furthermore, newer tables (`notes`, `day_plans`) were omitted from the deletion list. This resulted in orphaned data remaining in the database after a user "deleted" their account, violating GDPR "Right to Erasure" requirements.
**Learning:** Automated or generic cleanup loops are fragile when schemas evolve or relationships become complex. Tables lacking direct `user_id` columns must be explicitly handled.
**Prevention:** Prefer `ON DELETE CASCADE` on database foreign keys tied to the core `profiles` table to handle automatic cleanup, eliminating the need for hardcoded application-layer arrays. If manual deletion is required, explicitly address every foreign key relationship.
