...
## 2026-03-01 - Incomplete GDPR Data Deletion (Orphaned Records)
**Vulnerability:** The account deletion edge function iterated over a hardcoded array of tables and deleted records strictly using `.eq('user_id', userId)`. Tables that used different foreign key names (e.g., `owner_id`, `partner_id`, `user_id_1`, `shared_with_id`) for user relationships were either skipped or only partially cleaned up, resulting in orphaned sensitive data remaining in the database after a user requested account deletion (violating GDPR "Right to be Forgotten").
**Learning:** Data deletion routines must explicitly account for all foreign key relationships a user entity has across the entire schema, not just a generic `user_id` column.
**Prevention:** Always maintain a comprehensive map of all tables referencing the user entity when building manual deletion functions. Ensure tables lacking a direct `user_id` column (e.g., `note_shares`, `partnerships`, `budget_shares`, `teams`) are excluded from generic deletion loops and explicitly handled using their relevant foreign keys (and `.or()` syntax for dual-key tables).
