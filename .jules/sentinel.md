...
## 2025-03-09 - [GDPR Compliance: Foreign Key Account Deletion]
**Vulnerability:** Data Leak (GDPR/Account Deletion). Tables like `partnerships` and `budget_shares` did not contain a `user_id` column. Because the account deletion edge function hardcoded an array of tables to loop and delete by `user_id`, the Supabase client silently ignored the error in a catch block, leaving orphaned PII in the database when an account was deleted.
**Learning:** Hardcoded arrays looping `delete().eq('user_id')` must be manually updated and audited against schema changes. They will silently fail for tables relying on alternative foreign key names (e.g. `owner_id`, `partner_id`, `user_id_1`).
**Prevention:** Use `ON DELETE CASCADE` in the database schema where possible. For application-layer deletions, always verify schemas and explicitly handle edge case foreign keys using `.or()` to hit all relevant columns.
