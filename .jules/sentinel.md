...
## 2025-03-08 - GDPR Data Leak in Account Deletion Hardcoded Arrays
**Vulnerability:** Application-layer database deletion using a hardcoded `tablesToDelete` array failed to clear tables lacking a direct `user_id` column (e.g. partnerships, budget_shares) and missed newly added tables (day_plans, notes), causing a GDPR data leak where user data persisted after account deletion.
**Learning:** Hardcoded deletion arrays in Edge Functions are fragile and must be manually synchronized with every database schema change. Tables lacking standard `user_id` columns must be explicitly deleted using their specific foreign keys outside the generic deletion loop.
**Prevention:** Rely on PostgreSQL `ON DELETE CASCADE` foreign keys referencing the auth.users or profiles tables whenever possible. If manual deletion arrays are strictly required, implement schema drift tests to ensure newly created tables are handled.
