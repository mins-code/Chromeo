...
## 2026-06-04 - Incomplete Account Data Deletion
**Vulnerability:** The account deletion edge function only deleted records where `user_id` matches, leaving orphaned records in tables like `note_shares`, `partnerships`, `budget_shares`, and `teams` where the user is referenced by other keys (e.g., `owner_id`, `partner_id`). This violates GDPR right-to-be-forgotten and leaves orphaned data.
**Learning:** When deleting a user, all tables containing user references must be identified and explicitly deleted using all possible foreign keys, not just a generic `user_id` loop. Also, dependent child tables must be deleted before their parent tables to avoid foreign key constraints.
**Prevention:** Use `ON DELETE CASCADE` at the database level where possible, or ensure manual deletion scripts explicitly list and handle all relationship foreign keys in correct dependency order.
