...
## 2026-03-17 - IDOR in Task Operations
**Vulnerability:** The `taskService.ts` allowed users to update and delete tasks solely by their ID without verifying ownership. Since Supabase client libraries execute directly from the frontend, an attacker with a valid JWT could craft requests to modify or delete tasks belonging to other users if they knew or guessed the task ID.
**Learning:** Client-side service layers often abstract away the direct database queries, making it easy to forget that these queries are still subject to authorization rules. While Row Level Security (RLS) is the primary defense, relying solely on it without explicit client-side ownership checks (`.eq('user_id', user.id)`) creates a fragile defense-in-depth posture, especially if RLS policies are ever misconfigured or temporarily disabled.
**Prevention:** Always include explicit ownership filters (like `.eq('user_id', user.id)`) on all client-side mutation queries (update, delete) for user-owned resources, acting as a secondary layer of authorization alongside database RLS.

## 2026-03-17 - Overly Permissive CORS Configuration in Account Deletion Edge Function
**Vulnerability:** The `account-deletion` Edge Function configured CORS with `Access-Control-Allow-Origin: *`, which allows any website to make cross-origin requests to the endpoint. Since the endpoint handles sensitive account deletion actions via Authorization tokens, an overly permissive origin allows malicious sites to orchestrate cross-site requests (or makes it easier to test CSRF/XSS vectors) and could lead to unauthorized actions if authentication checks were somehow bypassed or if cache poisoning occurred.
**Learning:** Defaulting to `*` for CORS origins is a common pitfall. Edge Functions handling sensitive or state-changing operations should dynamically validate the `Origin` header against a strict whitelist of known-good origins. The `Vary: Origin` header is also critical when dynamically setting the allowed origin, to prevent intermediate caches from serving a CORS response meant for one origin to a different origin.
**Prevention:** Always implement strict CORS security by dynamically validating the request `Origin` against a whitelist of trusted domains (e.g., `APP_URL`, `localhost`). Set the `Access-Control-Allow-Origin` response header conditionally based on this validation, and always include `Vary: Origin` to prevent cache poisoning.
