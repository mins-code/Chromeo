...
serve(async (req) => {
  // Strict CORS Security dynamically validating the request Origin against whitelist
  ...
  try {
    ...
    if (limitError) {
      ...
      return new Response(
        ...
        { status: 503, headers: { ...headersWithCors, "Content-Type": "application/json" } }
      );
    }
    ...
    if (typeof currentCount === 'number' && currentCount > MAX_REQUESTS) {
        return new Response(
            ...
            { status: 429, headers: { ...headersWithCors, "Content-Type": "application/json" } }
        )
    }
