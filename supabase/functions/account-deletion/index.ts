...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "delete-with-password") {
      ...
      // Explicitly delete from tables with varied foreign key names AFTER dependent tables
      try {
        await supabase.from("note_shares").delete().or(`owner_id.eq.${userId},shared_with_id.eq.${userId}`);
      } catch (e) {
        console.log(`Explicit deletion cleanup error (note_shares): ${e}`);
      }
      try {
        await supabase.from("partnerships").delete().or(`user_id_1.eq.${userId},user_id_2.eq.${userId}`);
      } catch (e) {
        console.log(`Explicit deletion cleanup error (partnerships): ${e}`);
      }
      try {
        await supabase.from("budget_shares").delete().or(`owner_id.eq.${userId},partner_id.eq.${userId}`);
      } catch (e) {
        console.log(`Explicit deletion cleanup error (budget_shares): ${e}`);
      }
      try {
        await supabase.from("teams").delete().eq("owner_id", userId);
      } catch (e) {
        console.log(`Explicit deletion cleanup error (teams): ${e}`);
      }
