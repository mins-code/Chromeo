...
serve(async (req) => {
  ...
  try {
    ...
    switch (mode) {
      case 'chat':
        systemInstruction = `${BASE_SYSTEM_INSTRUCTION}\n\nIMPORTANT: The user's name is "${safeUserName}". Address them by name occasionally.${safeTagsContext}`;
        break;

      case 'enhance':
        systemInstruction = BASE_SYSTEM_INSTRUCTION + `\n\nEnsure output is strictly JSON with keys: description, subtasks (string array), priority, tags.${safeTagsContext}`;
        isJsonMode = true;
        break;
