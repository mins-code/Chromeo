...
serve(async (req) => {
  ...
  try {
    ...
    for (const item of recurringTx) {
        try {
            // 🛡️ SECURITY: Calculate next date FIRST to prevent partial failure
            // If this throws (e.g. invalid date), we skip the insert, preventing duplicates
            ...
            const nextDateStr = nextDate.toISOString(); // Validates date
            ...
            if (updateError) {
                 console.error(`CRITICAL: Failed to update recurring transaction ${item.id} after insert`, updateError);
                 // Note: Ideally we should rollback the insert here, but that requires RPC.
                 // For now, logging effectively signals the issue.
            } else {
                 results.transactions.push(item.id);
            }

        } catch (itemError) {
            console.error(`Error processing recurring transaction ${item.id}:`, itemError);
            // Continue to next item to prevent DoS
        }
    }
    ...
    for (const task of recurringTasks) {
        try {
            ...
            if (updateError) {
                 console.error(`CRITICAL: Failed to update recurring task ${task.id} after insert`, updateError);
            } else {
                 results.tasks.push(task.id);
            }

        } catch (itemError) {
            console.error(`Error processing recurring task ${task.id}:`, itemError);
