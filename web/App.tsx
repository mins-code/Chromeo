...
const App: React.FC = () => {
  ...
  const { notificationPermission, handleNotificationToggle, handleNotificationPreferenceChange } =
  ...
  // Morning Briefing
  const { showBriefing, overdueTasks, dismissBriefing } = useMorningBriefing({ tasks });
  ...
  const handleCommandBarTask = useCallback(
  ...
  const handleAutoCreatedTask = useCallback(
    async (taskData: Partial<Task>) => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return filtered.sort(
      (a, b) => (taskUrgencyScores.get(b.id) ?? 0) - (taskUrgencyScores.get(a.id) ?? 0)
    );
  ...
  const getHeaderInfo = () => {
    switch (currentView) {
      case 'reminders':
        return {
          title: t(theme, 'reminders'),
          subtitle: "Don't forget these important items",
          icon: Bell,
        };
      case 'events':
        return {
          title: t(theme, 'events'),
          subtitle: 'Upcoming social and work gatherings',
          icon: CalendarDays,
        };
      case 'appointments':
        return {
          title: t(theme, 'appointments'),
          subtitle: 'Scheduled meetings and visits',
          icon: Clock,
        };
      case 'tasks':
        return {
          title: t(theme, 'tasks'),
          subtitle: 'Manage and track your daily activities',
          icon: CheckSquare,
        };
      case 'all-activities':
        return {
          title: 'All Activities',
          subtitle: 'Tasks, Reminders, Events & Appointments',
          icon: Activity,
        };
      default:
        return { title: 'Items', subtitle: 'List View', icon: CheckSquare };
    }
  };
  ...
  return (
    <Layout
      ...
      calendarTags={
        currentView === 'calendar' && calendarVisibleTags.length > 0 ? calendarVisibleTags : allTags
      }
    ...
    >
      ...
      {lastSmsTransaction && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[60] bg-white dark:bg-slate-800 shadow-2xl rounded-2xl p-4 flex items-center gap-4 border border-brand-500 animate-slide-up max-w-sm w-full">
          <div
            className={`p-2 rounded-full ${lastSmsTransaction.type === 'expense' ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}
          >
            <MessageSquare size={20} />
          </div>
          <div className="flex-1">
            <p className="text-xs font-bold uppercase text-slate-400">
              New {lastSmsTransaction.type} Detected
            </p>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
              {lastSmsTransaction.description}
            </p>
            <p
              className={`text-lg font-bold ${lastSmsTransaction.type === 'expense' ? 'text-red-500' : 'text-emerald-500'}`}
            >
              {lastSmsTransaction.type === 'expense' ? '-' : '+'}
              {lastSmsTransaction.amount.toLocaleString('en-IN', {
                style: 'currency',
                currency: 'INR',
              })}
            </p>
          ...
          <button
            onClick={() => setLastSmsTransaction(null)}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-200 dark:border-white/10 animate-scale-in">
            <div className="p-6 bg-brand-500 text-white">
              ...
              <p className="opacity-90 mt-2 text-sm">
                The following scheduled payments are due for processing.
              </p>
            </div>
            <div className="p-4 sm:p-6 space-y-3 max-h-[50vh] overflow-y-auto">
              {dueRecurringItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/5 transition-all ${isItemProcessing ? 'opacity-60' : ''}`}
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span
                          className={`text-sm font-semibold ${item.type === 'income' ? 'text-emerald-500' : 'text-red-500'}`}
                        >
                          {item.type === 'income' ? '+' : '-'}
        ...
        <div className="space-y-6 h-full flex flex-col animate-fade-in">
          <div className="flex flex-col md:flex-row gap-5 justify-between items-start md:items-center border-b border-slate-200 dark:border-white/5 pb-6">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-1">{headerInfo.subtitle}</p>
            </div>
            ...
            {filteredTasks.length === 0 && (
              <div className="flex flex-col items-center justify-center py-24 text-slate-500 dark:text-slate-400 glass-panel rounded-2xl border border-dashed border-slate-200 dark:border-white/10">
                ...
                <p className="font-medium text-slate-600 dark:text-slate-300">
                  No {currentView} found matching your filters.
                </p>
                <Button
                  variant="ghost"
                  onClick={() => {
