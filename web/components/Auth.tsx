...
const Auth: React.FC<AuthProps> = ({ theme }) => {
  ...
  return (
    <div className={`min-h-screen w-full flex items-center justify-center p-4 relative overflow-hidden theme-${theme}`}>
      {/* Dynamic Background */}
      <div className="absolute inset-0 theme-bg"></div>

      {/* Main Auth Card */}
      <div className="relative w-full max-w-md z-10">
        <div className="bg-surface backdrop-blur-xl rounded-3xl shadow-2xl border-2 border-border overflow-hidden">
          {/* Header Section */}
          ...
          <div className="p-8">
            {/* Toggle Buttons */}
            <div className="flex rounded-xl bg-surface p-1.5 mb-8 border-2 border-border shadow-inner">
              <button
                type="button"
                onClick={() => {
                  setIsSignUp(false);
            ...
            <form onSubmit={handleAuth} className="space-y-5">
              {/* Email Input */}
              <div>
                <label className="block text-sm font-bold text-text mb-2 uppercase tracking-wide">
                  Email
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-border bg-surface text-text placeholder-text-secondary/50 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-300 outline-none font-medium"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 px-6 rounded-xl bg-gradient-to-r from-primary to-accent hover:from-primary-dark hover:to-accent-dark text-white font-black text-lg transition-all duration-300 hover:shadow-2xl hover:shadow-primary/30 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group relative overflow-hidden"
              >
                {/* Shimmer Effect */}
