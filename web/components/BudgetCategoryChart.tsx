...
> = ({ active, payload, formatCurrency = (v) => `₹${v.toLocaleString('en-IN')}` }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl p-3 shadow-lg min-w-[140px]">
      ...
      <p className="text-xs text-slate-500 mt-1">{data.percentage.toFixed(1)}% of total</p>
    </div>
  );
};
    ...
    <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-4">
      {payload.map((entry, index) => (
        <div key={`legend-${index}`} className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-slate-600 dark:text-slate-400">{entry.value}</span>
        </div>
      ))}
    </div>
  );
