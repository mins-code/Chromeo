...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // Memoize partner budget aggregations to prevent inline O(K*N) calculations on every render
    const { partnerIncome, partnerExpenses } = useMemo(() => {
        if (!expandedPartnerBudget) return { partnerIncome: 0, partnerExpenses: 0 };
        return expandedPartnerBudget.budget.transactions.reduce((acc, t) => {
            if (t.type === 'income') acc.partnerIncome += t.amount;
            else if (t.type === 'expense') acc.partnerExpenses += t.amount;
            return acc;
        }, { partnerIncome: 0, partnerExpenses: 0 });
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-lg font-bold ${(expandedPartnerBudget.budget.limit - partnerExpenses) >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {(expandedPartnerBudget.budget.limit - partnerExpenses)
