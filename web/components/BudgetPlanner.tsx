...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    const expandedPartnerTotals = useMemo(() => {
        ...
        const { income, expenses } = expandedPartnerBudget.budget.transactions.reduce(
            (acc, curr) => {
        ...
        const rem = expandedPartnerBudget.budget.limit - expenses;
        const progress = Math.min(100, (expenses / (expandedPartnerBudget.budget.limit || 1)) * 100);
        return { income, expenses, remaining: rem, progress };
                                    ...
                                    <div className="ml-11 p-4 rounded-xl bg-gradient-to-br from-emerald-500/5 to-brand-500/5 border border-emerald-500/10 space-y-3">
                                        ...
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                ...
                                                style={{ 
                                                    width: `${expandedPartnerTotals.progress}%`
                                                }}
                                            />
                                        </div>
                                    </div>
