...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
  ...
  const [expandedPartnerBudget, setExpandedPartnerBudget] = useState<{
    ownerId: string;
    budget: Budget;
  } | null>(null);
  ...
  const partnerBudgetStats = useMemo(() => {
    if (!expandedPartnerBudget) return { income: 0, expenses: 0, remaining: 0, progress: 0 };

    let income = 0;
    let expenses = 0;
    const limit = expandedPartnerBudget.budget.limit || 0;
    const txs = expandedPartnerBudget.budget.transactions || [];

    for (let i = 0; i < txs.length; i++) {
      if (txs[i].type === 'income') {
        income += txs[i].amount;
      } else if (txs[i].type === 'expense') {
        expenses += txs[i].amount;
      }
    }

    const remaining = limit - expenses;
    const progress = Math.min(100, (expenses / (limit || 1)) * 100);

    return { income, expenses, remaining, progress };
    ...
    <div className="space-y-8 animate-fade-in h-full flex flex-col">
      <div className="border-b border-slate-200 dark:border-white/5 pb-6">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Track your {t(currentTheme, 'income').toLowerCase()}, expenses, and savings goals.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-20">
        ...
        <div className="lg:col-span-2 glass-panel p-8 rounded-3xl relative overflow-hidden flex flex-col justify-center min-h-[220px]">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 relative z-10">
            <div>
              <p className="text-[10px] font-bold uppercase text-slate-400 mb-1 tracking-widest font-mono">
                {t(currentTheme, 'totalBudget')}
              </p>
            ...
            <div className="relative group">
              <p className="text-[10px] font-bold uppercase text-slate-400 mb-1 tracking-widest font-mono flex items-center gap-2">
                ...
                <button
                  ...
                  aria-label={showSavings ? 'Hide savings' : 'Show savings'}
                >
                  {showSavings ? <EyeOff size={12} /> : <Eye size={12} />}
                </button>
              </p>
              <p
                className={`text-xl sm:text-2xl font-bold ${totalIncome - totalExpenses >= 0 ? 'text-blue-500' : 'text-red-500'}`}
              >
            ...
            <div>
              ...
              <p
                className={`text-xl sm:text-2xl font-bold ${dailySafeSpend >= 0 ? 'text-cyan-500' : 'text-red-500'}`}
              >
        ...
        <div className="glass-panel p-6 rounded-3xl space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase text-slate-400 ml-1 font-mono">
                Duration
              </label>
              <Select
                value={durationInput}
            ...
            <Button variant="primary" className="w-full" onClick={handleUpdateSettings}>
              Update Budget
            </Button>
          </div>
        </div>
      ...
      <div className="glass-panel p-6 rounded-3xl space-y-4">
        ...
        {budget.recurring.length > 0 && (
          <p className="text-sm text-slate-500 dark:text-slate-400 text-center">
            ...
            <span
              className={`font-bold ${projectedEndBalance >= 0 ? 'text-brand-500' : 'text-red-500'}`}
            >
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-slate-200 dark:border-white/5">
          ...
          {isRecurring && (
            <Select
              ...
              onChange={(value) =>
                setRecurringFrequency(value as 'daily' | 'weekly' | 'monthly' | 'yearly')
              }
              options={[
                { value: 'daily', label: 'Daily' },
                { value: 'weekly', label: 'Weekly' },
          ...
          {isRecurring && (
            <div className="flex flex-col">
              <label className="text-[10px] font-bold uppercase text-slate-400 font-mono mb-1">
                Start Date
              </label>
              <input
                type="date"
      ...
      <div className="glass-panel p-6 rounded-3xl space-y-4 relative z-0">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Share your budget overview with connected partners so they can view your spending
          progress.
        </p>
              ...
              <div key={sharedBudget.shareId} className="space-y-2">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-brand-500">
                    {isLoadingPartnerBudget &&
                    expandedPartnerBudget?.ownerId !== sharedBudget.ownerId ? null : (
                      <>
                        ...
                        <ArrowRight
                          size={14}
                          className={`transition-transform ${expandedPartnerBudget?.ownerId === sharedBudget.ownerId ? 'rotate-90' : ''}`}
                        />
                      </>
                    )}
                    {isLoadingPartnerBudget &&
                ...
                {expandedPartnerBudget?.ownerId === sharedBudget.ownerId && (
                  <div className="ml-11 p-4 rounded-xl bg-gradient-to-br from-emerald-500/5 to-brand-500/5 border border-emerald-500/10 space-y-3">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      <div>
                        <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">
                          Budget
                        </p>
                        <p className="text-lg font-bold text-slate-800 dark:text-slate-100">
                          {expandedPartnerBudget.budget.limit.toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            maximumFractionDigits: 0,
                          })}
                        ...
                        <p className="text-[10px] text-slate-500 font-mono">
                          / {expandedPartnerBudget.budget.duration}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">
                        ...
                        <p className="text-lg font-bold text-red-500">
                          {partnerBudgetStats.expenses.toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            maximumFractionDigits: 0,
                          })}
                      ...
                      <div>
                        ...
                        <p
                          className={`text-lg font-bold ${partnerBudgetStats.remaining >= 0 ? 'text-brand-500' : 'text-red-500'}`}
                        >
                          {partnerBudgetStats.remaining.toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            maximumFractionDigits: 0,
                          })}
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-emerald-500">
                          {partnerBudgetStats.income.toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            maximumFractionDigits: 0,
                          })}
                    ...
                    <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                      <div
                        className={`h-full transition-all duration-500 ${
                          partnerBudgetStats.remaining < 0 ? 'bg-red-500' : 'bg-brand-500'
                        }`}
                        style={{
                          width: `${partnerBudgetStats.progress}%`,
                        }}
                      />
                    </div>
                  </div>
