...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // Partner budget calculations in a single pass to avoid redundant O(K*N) passes in JSX
    const { partnerExpenses, partnerIncome, partnerRemaining } = useMemo(() => {
        if (!expandedPartnerBudget) return { partnerExpenses: 0, partnerIncome: 0, partnerRemaining: 0 };
        ...
        return {
            partnerExpenses: totals.expenses,
            partnerIncome: totals.income,
            partnerRemaining: expandedPartnerBudget.budget.limit - totals.expenses
        };
