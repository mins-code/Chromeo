                        ...
                        {budgetsSharedWithMe.map(sharedBudget => (
                            <div key={sharedBudget.shareId} className="space-y-2">
                                ...
                                {expandedPartnerBudget?.ownerId === sharedBudget.ownerId && (() => {
                                    const stats = expandedPartnerBudget.budget.transactions.reduce((acc, t) => {
                                        if (t.type === 'expense') acc.expenses += t.amount;
                                        else if (t.type === 'income') acc.income += t.amount;
                                        return acc;
                                    }, { expenses: 0, income: 0 });
                                    const remaining = expandedPartnerBudget.budget.limit - stats.expenses;
                                    const progressPercentage = Math.min(100, (stats.expenses / (expandedPartnerBudget.budget.limit || 1)) * 100);
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-lg font-bold text-red-500">
                                                        {stats.expenses.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                    </p>
                                                </div>
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-lg font-bold text-emerald-500">
                                                        {stats.income.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                    </p>
                                                </div>
                                            </div>
                                            {/* Progress bar */}
