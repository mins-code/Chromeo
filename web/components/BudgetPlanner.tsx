...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // Calculate expanded partner budget totals in a single pass to avoid O(K*N) inline reductions
    const expandedPartnerTotals = useMemo(() => {
        if (!expandedPartnerBudget) return { expenses: 0, income: 0, limit: 0, remaining: 0 };
        ...
        return {
            expenses,
            income,
            limit: expandedPartnerBudget.budget.limit,
            remaining: expandedPartnerBudget.budget.limit - expenses
        };
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-slate-800 dark:text-slate-100">
                                                    {expandedPartnerTotals.limit.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-lg font-bold ${expandedPartnerTotals.remaining >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {expandedPartnerTotals.remaining.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                        ...
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    expandedPartnerTotals.remaining < 0
                                                        ? 'bg-red-500' 
                                                        : 'bg-brand-500'
                                                }`}
                                                style={{ 
                                                    width: `${Math.min(100, (expandedPartnerTotals.expenses / (expandedPartnerTotals.limit || 1)) * 100)}%`
                                                }}
                                            />
                                        </div>
                                    </div>
