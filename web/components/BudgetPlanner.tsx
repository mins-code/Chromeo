...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    const expandedPartnerExpenses = useMemo(() => {
        if (!expandedPartnerBudget) return 0;
        return expandedPartnerBudget.budget.transactions.reduce((acc, t) => t.type === 'expense' ? acc + t.amount : acc, 0);
    }, [expandedPartnerBudget]);

    const expandedPartnerIncome = useMemo(() => {
        if (!expandedPartnerBudget) return 0;
        return expandedPartnerBudget.budget.transactions.reduce((acc, t) => t.type === 'income' ? acc + t.amount : acc, 0);
    }, [expandedPartnerBudget]);

    const expandedPartnerRemaining = useMemo(() => {
        if (!expandedPartnerBudget) return 0;
        return expandedPartnerBudget.budget.limit - expandedPartnerExpenses;
    }, [expandedPartnerBudget, expandedPartnerExpenses]);
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-red-500">
                                                    {expandedPartnerExpenses.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Remaining</p>
                                                <p className={`text-lg font-bold ${expandedPartnerRemaining >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {expandedPartnerRemaining.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Income</p>
                                                <p className="text-lg font-bold text-emerald-500">
                                                    {expandedPartnerIncome.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                        </div>
                                        {/* Progress bar */}
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    expandedPartnerRemaining < 0
                                                        ? 'bg-red-500' 
                                                        : 'bg-brand-500'
                                                }`}
                                                style={{ 
                                                    width: `${Math.min(100, (expandedPartnerExpenses / (expandedPartnerBudget.budget.limit || 1)) * 100)}%`
                                                }}
                                            />
                                        </div>
                                    </div>
