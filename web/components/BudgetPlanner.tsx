...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    const { expandedPartnerIncome, expandedPartnerExpenses } = useMemo(() => {
        if (!expandedPartnerBudget?.budget?.transactions) {
            return { expandedPartnerIncome: 0, expandedPartnerExpenses: 0 };
        }
        return expandedPartnerBudget.budget.transactions.reduce((acc, curr) => {
            if (curr.type === 'income') acc.expandedPartnerIncome += curr.amount;
            else if (curr.type === 'expense') acc.expandedPartnerExpenses += curr.amount;
            return acc;
        }, { expandedPartnerIncome: 0, expandedPartnerExpenses: 0 });
                                    ...
                                    <div className="ml-11 p-4 rounded-xl bg-gradient-to-br from-emerald-500/5 to-brand-500/5 border border-emerald-500/10 space-y-3">
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-lg font-bold ${(expandedPartnerBudget.budget.limit - expandedPartnerExpenses) >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {(expandedPartnerBudget.budget.limit - expandedPartnerExpenses).toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                        ...
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    (expandedPartnerBudget.budget.limit - expandedPartnerExpenses) < 0
                                                        ? 'bg-red-500' 
                                                        : 'bg-brand-500'
                                                }`}
                                                style={{ 
