...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // Optimize expanded partner budget calculations
    const partnerTotals = useMemo(() => {
        if (!expandedPartnerBudget) return { income: 0, expenses: 0, remaining: 0, progress: 0 };
        const totals = expandedPartnerBudget.budget.transactions.reduce((acc, t) => {
            if (t.type === 'income') acc.income += t.amount;
            else if (t.type === 'expense') acc.expenses += t.amount;
            return acc;
        }, { income: 0, expenses: 0 });
        const remaining = expandedPartnerBudget.budget.limit - totals.expenses;
        const progress = Math.min(100, (totals.expenses / (expandedPartnerBudget.budget.limit || 1)) * 100);
        return { ...totals, remaining, progress };
