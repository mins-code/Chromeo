                        ...
                        {budgetsSharedWithMe.map(sharedBudget => (
                            <div key={sharedBudget.shareId} className="space-y-2">
                                ...
                                {expandedPartnerBudget?.ownerId === sharedBudget.ownerId && (() => {
                                    const partnerTotalExpenses = expandedPartnerBudget.budget.transactions
                                        .filter(t => t.type === 'expense')
                                        .reduce((acc, t) => acc + t.amount, 0);
                                    const partnerTotalIncome = expandedPartnerBudget.budget.transactions
                                        .filter(t => t.type === 'income')
                                        .reduce((acc, t) => acc + t.amount, 0);
                                    const partnerRemaining = expandedPartnerBudget.budget.limit - partnerTotalExpenses;
                                    ...
                                    return (
                                        <div className="ml-11 p-4 rounded-xl bg-gradient-to-br from-emerald-500/5 to-brand-500/5 border border-emerald-500/10 space-y-3">
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-lg font-bold text-red-500">
                                                        {partnerTotalExpenses.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                    </p>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Remaining</p>
                                                    <p className={`text-lg font-bold ${partnerRemaining >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-lg font-bold text-emerald-500">
                                                        {partnerTotalIncome.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                    </p>
                                                </div>
                                            </div>
                                            {/* Progress bar */}
                                            <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full transition-all duration-500 ${partnerRemaining < 0 ? 'bg-red-500' : 'bg-brand-500'}`}
                                                    style={{ width: `${Math.min(100, (partnerTotalExpenses / (expandedPartnerBudget.budget.limit || 1)) * 100)}%` }}
                                                />
                                            </div>
                                        </div>
                                    );
