...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    const partnerTotals = useMemo(() => {
        if (!expandedPartnerBudget) return { expenses: 0, income: 0, remaining: 0, progress: 0 };
        const totals = expandedPartnerBudget.budget.transactions.reduce(
            (acc, curr) => {
                if (curr.type === 'expense') acc.expenses += curr.amount;
                else if (curr.type === 'income') acc.income += curr.amount;
                return acc;
            },
            { expenses: 0, income: 0 }
        );
        const rem = expandedPartnerBudget.budget.limit - totals.expenses;
        const prog = Math.min(100, (totals.expenses / (expandedPartnerBudget.budget.limit || 1)) * 100);
        return { ...totals, remaining: rem, progress: prog };
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-red-500">
                                                    {partnerTotals.expenses.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Remaining</p>
                                                <p className={`text-lg font-bold ${partnerTotals.remaining >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {partnerTotals.remaining.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Income</p>
                                                <p className="text-lg font-bold text-emerald-500">
                                                    {partnerTotals.income.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                        </div>
                                        {/* Progress bar */}
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    partnerTotals.remaining < 0 ? 'bg-red-500' : 'bg-brand-500'
                                                }`}
                                                style={{ width: `${partnerTotals.progress}%` }}
                                            />
                                        </div>
                                    </div>
                                )}
