...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // Pre-calculate expanded partner budget stats to avoid repeated inline map/reduce in render
    const { partnerExpenses, partnerIncome } = useMemo(() => {
        if (!expandedPartnerBudget) return { partnerExpenses: 0, partnerIncome: 0 };
        return expandedPartnerBudget.budget.transactions.reduce(
            (acc, t) => {
                if (t.type === 'expense') acc.partnerExpenses += t.amount;
                else if (t.type === 'income') acc.partnerIncome += t.amount;
                return acc;
            },
            { partnerExpenses: 0, partnerIncome: 0 }
        );
                                    ...
                                    <div className="ml-11 p-4 rounded-xl bg-gradient-to-br from-emerald-500/5 to-brand-500/5 border border-emerald-500/10 space-y-3">
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-lg font-bold ${(expandedPartnerBudget.budget.limit - partnerExpenses) >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {(expandedPartnerBudget.budget.limit - partnerExpenses).toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                        ...
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    (expandedPartnerBudget.budget.limit - partnerExpenses) < 0
                                                        ? 'bg-red-500' 
                                                        : 'bg-brand-500'
                                                }`}
                                                style={{ 
                                                    width: `${Math.min(100, (partnerExpenses / (expandedPartnerBudget.budget.limit || 1)) * 100)}%`
                                                }}
                                            />
                                        </div>
                                    </div>
