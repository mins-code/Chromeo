...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    const partnerTotalExpenses = useMemo(() => {
        if (!expandedPartnerBudget) return 0;
        return expandedPartnerBudget.budget.transactions
            .filter(t => t.type === 'expense')
            .reduce((acc, t) => acc + t.amount, 0);
    }, [expandedPartnerBudget]);

    const partnerTotalIncome = useMemo(() => {
        if (!expandedPartnerBudget) return 0;
        return expandedPartnerBudget.budget.transactions
    ...
    const partnerRemaining = useMemo(() => {
        if (!expandedPartnerBudget) return 0;
        return expandedPartnerBudget.budget.limit - partnerTotalExpenses;
    }, [expandedPartnerBudget, partnerTotalExpenses]);
