...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // Memoize expanded partner budget totals
    const expandedPartnerTotals = useMemo(() => {
        if (!expandedPartnerBudget) return { income: 0, expense: 0, remaining: 0 };
        const { income, expense } = expandedPartnerBudget.budget.transactions.reduce((acc, curr) => {
            if (curr.type === 'income') acc.income += curr.amount;
            else if (curr.type === 'expense') acc.expense += curr.amount;
            return acc;
        }, { income: 0, expense: 0 });
        return { income, expense, remaining: expandedPartnerBudget.budget.limit - expense };
                                    ...
                                    <div className="ml-11 p-4 rounded-xl bg-gradient-to-br from-emerald-500/5 to-brand-500/5 border border-emerald-500/10 space-y-3">
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-red-500">
                                                    {expandedPartnerTotals.expense.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                        ...
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                ...
                                                style={{ 
                                                    width: `${Math.min(100, (expandedPartnerTotals.expense / (expandedPartnerBudget.budget.limit || 1)) * 100)}%`
                                                }}
                                            />
                                        </div>
                                    </div>
