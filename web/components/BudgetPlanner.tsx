...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    const partnerTotals = useMemo(() => {
        if (!expandedPartnerBudget?.budget?.transactions) return { income: 0, expense: 0 };
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-red-500">
                                                    {partnerTotals.expense.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Remaining</p>
                                                <p className={`text-lg font-bold ${(expandedPartnerBudget.budget.limit - partnerTotals.expense) >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {(expandedPartnerBudget.budget.limit - partnerTotals.expense)
                                        ...
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    (expandedPartnerBudget.budget.limit - partnerTotals.expense) < 0
                                                        ? 'bg-red-500' 
                                                        : 'bg-brand-500'
                                                }`}
                                                style={{ 
                                                    width: `${Math.min(100, (partnerTotals.expense / (expandedPartnerBudget.budget.limit || 1)) * 100)}%`
                                                }}
                                            />
                                        </div>
                                    </div>
