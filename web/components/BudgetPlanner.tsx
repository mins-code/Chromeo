...
const BudgetPlanner: React.FC<BudgetPlannerProps> = ({ currentTheme }) => {
    ...
    // ⚡ Performance Optimization: Memoize expanded partner budget totals to avoid redundant O(N) array filtering and reduction on every render (used 6 times in JSX)
    const expandedPartnerTotals = useMemo(() => {
        if (!expandedPartnerBudget) return { income: 0, expenses: 0 };
        return expandedPartnerBudget.budget.transactions.reduce((acc, curr) => {
            if (curr.type === 'income') acc.income += curr.amount;
            else if (curr.type === 'expense') acc.expenses += curr.amount;
            return acc;
        }, { income: 0, expenses: 0 });
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-red-500">
                                                    {expandedPartnerTotals.expenses.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Remaining</p>
                                                <p className={`text-lg font-bold ${(expandedPartnerBudget.budget.limit - expandedPartnerTotals.expenses) >= 0 ? 'text-brand-500' : 'text-red-500'}`}>
                                                    {(expandedPartnerBudget.budget.limit - expandedPartnerTotals.expenses).toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 font-mono">Income</p>
                                                <p className="text-lg font-bold text-emerald-500">
                                                    {expandedPartnerTotals.income.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                                                </p>
                                            </div>
                                        </div>
                                        {/* Progress bar */}
                                        <div className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-500 ${
                                                    (expandedPartnerBudget.budget.limit - expandedPartnerTotals.expenses) < 0
                                                        ? 'bg-red-500' 
                                                        : 'bg-brand-500'
                                                }`}
                                                style={{ 
                                                    width: `${Math.min(100, (expandedPartnerTotals.expenses / (expandedPartnerBudget.budget.limit || 1)) * 100)}%`
                                                }}
                                            />
                                        </div>
                                    </div>
