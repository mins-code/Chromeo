...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !intervalDropdownRef.current.contains(event.target as Node)
      ) {
  ...
  const customIntervalDays = useMemo(() => {
    switch (customIntervalUnit) {
      case 'week':
        return customIntervalValue * 7;
      case 'month':
        return customIntervalValue * 30; // Approximate
      default:
        ...
        if (frequency === 'daily') {
          ...
          while (candidate <= rangeEnd) {
            if (candidate >= rangeStart && candidate.getDate() === taskDate.getDate()) {
              // Check interval
              const monthDiff =
                (candidate.getFullYear() - taskDate.getFullYear()) * 12 +
                (candidate.getMonth() - taskDate.getMonth());
              if (
                monthDiff % recurrenceInterval === 0 &&
                (!taskEndDate || candidate <= taskEndDate)
              ) {
                hasOccurrenceInRange = true;
                break;
              }
  ...
  useEffect(() => {
    if (onVisibleTagsChange) {
      ...
      const isSame =
        visibleTags.length === prevVisibleTagsRef.current.length &&
  ...
  const navigate = (direction: number) => {
    if (viewMode === 'month') {
      ...
      setCurrentDate(
        direction > 0
          ? addDays(currentDate, customIntervalDays)
          : subDays(currentDate, customIntervalDays)
      );
    }
  };
  ...
  const checkRecurrence = useCallback(
    (task: Task, taskDate: Date, date: Date, endDate?: Date | null): boolean => {
      const isSameDay =
      ...
      if (frequency === 'monthly') {
        ...
        const monthDiff =
          (date.getFullYear() - taskDate.getFullYear()) * 12 +
    ...
    tasks.forEach((task) => {
      ...
      if (!task.recurrence || task.recurrence.frequency === 'none') {
        ...
        if (frequency === 'daily' && interval === 1) {
          ...
          if (
            taskEndDate &&
            taskEndDate.getFullYear() === year &&
            taskEndDate.getMonth() === month
          ) {
    ...
    const updatedTask: Task = {
      ...
      reminderTime: task.reminderTime
        ? new Date(
            ...
            newDate.getMonth(),
            newDate.getDate(),
            new Date(task.reminderTime).getHours(),
            new Date(task.reminderTime).getMinutes()
          ).toISOString()
        : undefined,
    };
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col animate-fade-in relative">
        ...
        {viewMode === 'day' && (
          <DayView tasks={tasks} currentDate={currentDate} onEditTask={onEditTask} />
        )}
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md glass rounded-2xl shadow-2xl overflow-hidden animate-scale-in flex flex-col max-h-[70vh]">
              <div className="p-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between bg-white/50 dark:bg-slate-900/50">
                ...
                <button
                  onClick={() => setSelectedDate(null)}
                  className="p-1 rounded-full hover:bg-black/5 dark:hover:bg-white/5 text-slate-400 hover:text-slate-800 dark:hover:text-white"
                  aria-label="Close"
                >
                  <X size={20} />
                </button>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        >
                          <h4
                            className={`font-semibold text-sm ${isDone ? 'line-through text-slate-400 dark:text-slate-500' : ''}`}
                          >
                            {task.title}
                          </h4>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-800 dark:text-slate-100">
                      {formatEventTime(selectedGoogleEvent)}
                    </p>
        ...
        <DragOverlay>
          {activeTask && (
            <div
              className={`px-3 py-2 rounded-lg shadow-lg border-l-4 ${TYPE_COLORS[activeTask.type]}`}
            >
