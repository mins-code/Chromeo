    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {routines.length > 0 ? (
        ...
        <div className="text-center py-16 glass-panel rounded-2xl border border-dashed border-slate-200 dark:border-white/10">
          ...
          <p className="text-slate-600 dark:text-slate-300 mb-6 max-w-sm mx-auto">
            Create routines for recurring activities like workout schedules, work hours, or daily
            habits.
          </p>
          <Button variant="primary" onClick={onCreate}>
            <Plus size={18} className="mr-2" />
      ...
      {routines.length > 0 && (
        <div className="glass-panel rounded-2xl p-6">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-4">
            Today's Routines
          </h3>
...
const TodayRoutinesPreview: React.FC<TodayRoutinesPreviewProps> = React.memo(({ routines }) => {
  ...
  if (todayRoutines.length === 0) {
    return (
      <p className="text-slate-500 dark:text-slate-400 text-sm">No routines scheduled for today.</p>
    );
  }
  ...
  return (
    <div className="flex flex-wrap gap-3">
      {todayRoutines.map((r, idx) => (
        ...
        >
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: r.color || '#10B981' }} />
          <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
            {r.displayName}
          </span>
          {r.routine.time && <span className="text-xs text-slate-500">@ {r.routine.time}</span>}
        </div>
      ))}
    </div>
  );
