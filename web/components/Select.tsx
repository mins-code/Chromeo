...
const Select: React.FC<SelectProps> = ({
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        <span
          className={`block truncate text-sm font-medium ${selectedOption ? colors.text : 'text-slate-500'}`}
        >
          {selectedOption ? selectedOption.label : placeholder}
        </span>
        <span
          className={`ml-2 flex items-center transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
        >
          {isOpen ? (
