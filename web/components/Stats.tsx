...
const Stats: React.FC<StatsProps> = ({ tasks, budget, onNavigate }) => {
  ...
  const todayStats = useMemo(() => {
    ...
    const lowPriority = todaysTasks.filter(
      (t) => t.priority === TaskPriority.LOW && t.status !== TaskStatus.DONE
    ).length;
    const mediumPriority = todaysTasks.filter(
      (t) => t.priority === TaskPriority.MEDIUM && t.status !== TaskStatus.DONE
    ).length;
    const highPriority = todaysTasks.filter(
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div
                className={`p-3 rounded-xl ${dailySafeSpend.isHealthy ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}
              >
                <Wallet
                  size={24}
                  className={dailySafeSpend.isHealthy ? 'text-emerald-500' : 'text-red-500'}
                />
              </div>
              <div>
                ...
                <p
                  className={`text-3xl font-bold ${dailySafeSpend.isHealthy ? 'text-emerald-500' : 'text-red-500'}`}
                >
                  ₹
                  {Math.abs(dailySafeSpend.amount).toLocaleString('en-IN', {
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-slate-400">
                ₹{dailySafeSpend.remaining.toLocaleString('en-IN', { maximumFractionDigits: 0 })}{' '}
                remaining
              </p>
            </div>
          </div>
