    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-x-2 group-hover:translate-x-0">
            <Button variant="secondary" size="icon" aria-label="Edit task" className="h-8 w-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur" onClick={(e) => { e.stopPropagation(); onEdit(task); }}>
            ...
            {onAIAnalysis && !isDone && (
                <Button 
                    ...
                    title="AI Breakdown"
                    aria-label="Generate AI breakdown for task"
                >
                    <Sparkles size={14} />
                </Button>
            )}
