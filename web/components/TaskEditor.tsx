...
const TaskEditor: React.FC<TaskEditorProps> = ({
  ...
  const [customNotificationUnit, setCustomNotificationUnit] = useState<
    'minutes' | 'hours' | 'days'
  >('minutes');
  const [notificationSoundId, setNotificationSoundId] = useState<NotificationSound | undefined>(
    undefined
  );
  ...
  const suggestedTags = React.useMemo(() => {
    ...
    return existingTags.filter((tag) => tag.includes(newTag.trim()) && !tags.includes(tag));
  ...
  useEffect(() => {
    if (task) {
      ...
      setReminderTime(
        task.reminderTime
          ? formatDateTimeLocal(task.reminderTime)
          : task.dueDate
      ...
      if (task.notificationTime) {
        ...
        if (
          ...
          ![5, 15, 60, 720, 1440].includes(task.notificationMinutesBefore)
        ) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      notificationMinutesBefore:
        notificationMode === 'relative' ? notificationMinutesBefore : undefined,
      notificationTime:
        notificationMode === 'absolute' && notificationTime
          ? new Date(notificationTime).toISOString()
          : undefined,
      notificationSoundId,
    });
    onClose();
  ...
  const handleAddTag = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      if (suggestedTags.length > 0) {
        ...
        setSelectedSuggestionIndex((prev) => (prev < suggestedTags.length - 1 ? prev + 1 : 0));
      }
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (suggestedTags.length > 0) {
        setShowTagSuggestions(true);
        setSelectedSuggestionIndex((prev) => (prev > 0 ? prev - 1 : suggestedTags.length - 1));
      }
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-7">
          ...
          <div className="relative">
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2 ml-1 flex items-center gap-2">
              Tags
              {newTag && (
                <span className="text-[10px] normal-case font-normal text-brand-500 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="bg-brand-500/10 text-brand-600 dark:text-brand-300 text-xs font-medium px-2.5 py-1 rounded-md flex items-center gap-1.5 border border-brand-500/20"
                >
                  {tag}
                  <button
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <Input
                  ...
                  aria-controls={
                    showTagSuggestions && suggestedTags.length > 0 ? listboxId : undefined
                  }
                  ...
                  {notificationMode === 'relative' ? (
                    ...
                    <div className="animate-fade-in">
                      ...
                      <p className="mt-2 text-xs text-amber-600/80 dark:text-amber-400/80">
                        This notification time is fixed and won&apos;t change even if you reschedule
                        the task.
                      </p>
                    </div>
                  )}
          ...
          {reminderTime && (notificationEnabled === undefined || notificationEnabled === true) && (
            <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-2xl border border-amber-200 dark:border-amber-500/20 space-y-3">
              <div className="flex items-center gap-2 text-sm font-medium text-amber-700 dark:text-amber-300">
                <Bell size={16} className="text-amber-500" />
                Notification Sound
                <span className="text-[10px] font-normal text-amber-500/70 dark:text-amber-400/60">
                  Android
                </span>
              </div>
              <Select
              ...
              {notificationSoundId === 'sound_custom_os' && isNativePlatform() && (
                <div className="mt-2 space-y-2">
                  ...
                  <p className="text-xs text-amber-600/80 dark:text-amber-400/80 text-center">
                    Tap this, select &apos;Notifications&apos;, tap &apos;Custom OS Alert&apos;, and
          ...
          <div>
            ...
            <div className="max-h-32 overflow-y-auto bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl p-2 space-y-1">
              {availableTasks.length === 0 && (
                <p className="text-xs text-slate-500 p-2">No other tasks available to link.</p>
              )}
              {availableTasks
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-xs font-medium px-2 py-1 rounded hover:bg-black/5 dark:hover:bg-white/5 flex items-center gap-1 text-brand-600 dark:text-brand-400 transition-colors"
              >
            ...
            <div className="space-y-2.5">
              {subtasks.map((st, index) => (
                <div
                  key={st.id}
                  className="flex items-center gap-3 group bg-slate-50 dark:bg-black/20 p-2 rounded-lg border border-transparent hover:border-slate-200 dark:hover:border-white/5 transition-all"
                >
                  <input
                    type="checkbox"
