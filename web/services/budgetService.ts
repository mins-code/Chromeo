...
export const updateTransaction = async (
  ...
  if (user) {
    ...
    await supabase.from('transactions').update(updateData).eq('id', id).eq('user_id', user.id); // Security: only update own transactions
  }
  return getBudget();
};
...
export const deleteTransaction = async (id: string): Promise<Budget> => {
  ...
  if (user) {
    await supabase.from('transactions').delete().eq('id', id).eq('user_id', user.id); // Security: only delete own transactions
  }
  return getBudget();
};
...
export const unshareBudgetWithPartner = async (shareId: string): Promise<boolean> => {
  const { error } = await supabase.from('budget_shares').delete().eq('id', shareId);
