...
const CATEGORY_PATTERNS: { pattern: RegExp; category: TransactionCategory }[] = [
  ...
  {
    pattern:
      /starbucks|mcdonald|burger|pizza|zomato|swiggy|domino|kfc|subway|cafe|restaurant|food|dining|eat|meal/i,
    category: 'Food & Dining',
  },
  // Transportation
  {
    pattern: /uber|ola|rapido|metro|bus|cab|taxi|fuel|petrol|diesel|parking|toll|lyft/i,
    category: 'Transportation',
  },
  // Shopping
  {
    pattern: /amazon|flipkart|myntra|ajio|shop|mall|store|retail|purchase|buy|order/i,
    category: 'Shopping',
  },
  // Utilities
  {
    pattern: /electric|water|gas|bill|recharge|mobile|airtel|jio|vodafone|wifi|internet|broadband/i,
    category: 'Utilities',
  },
  // Entertainment
  {
    pattern: /netflix|spotify|prime|hotstar|movie|cinema|theatre|game|play|concert|event|disney/i,
    category: 'Entertainment',
  },
  // Health
  {
    pattern: /hospital|doctor|medicine|pharmacy|medical|health|clinic|dental|eye|gym|fitness/i,
    category: 'Health',
  },
  // Travel
  {
    pattern: /flight|hotel|booking|makemytrip|goibibo|airbnb|train|irctc|trip|vacation|travel/i,
    category: 'Travel',
  },
  // Income
  {
    pattern: /salary|payment received|credited|deposit|refund|cashback|bonus|income/i,
    category: 'Income',
  },
];
...
export const enhanceTaskWithAI = async (
  ...
  try {
    ...
    const enrichedTask = {
      ...
      subtasks: result.subtasks.map((t: string) => ({
...
export const parseTransactionScreenshot = async (file: File): Promise<ScannedTransaction[]> => {
  try {
    ...
    if (!Array.isArray(parsed)) {
      ...
      throw new Error(
        'Could not extract transactions from this image. Please try a different screenshot.'
      );
    }

    if (parsed.length === 0) {
      throw new Error(
        'No transactions found in this image. Please ensure it shows a valid UPI receipt.'
      );
    }
