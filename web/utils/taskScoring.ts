...
export function sortByUrgency(tasks: Task[]): Task[] {
  // ⚡ Bolt Optimization: Schwartzian transform for O(N) score calculation instead of O(N log N)
  // This caches the urgency score so getUrgencyScore() (which does date math/parsing)
  // is only called once per task instead of multiple times during sort comparisons.
  const now = Date.now();
  return tasks
